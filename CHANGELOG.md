...
### Added
- **Read receipts** — messages now show who has read them. Direct messages
  display "Seen"; group channels display "Seen by N" with the reader names on
  hover. Both are powered by the same data and update in realtime.
  - New `GET /api/v1/channels/:id/receipts` returns each member's read cursor
    (`user_id`, `last_read_at`) for members of the channel.
  - New `read` WebSocket event (`{ channel_id, user_id, last_read_at }`) is
    emitted whenever a member advances their read cursor, so "Seen" indicators
    update the instant the other person reads.
  - Integration test covering the receipts endpoint and member-only access.
...
- Removed the unused `unreadCount()` client wrapper (the Chat-button count has
  been derived from per-channel unread since 0.2.0; the wrapper invited
  reintroducing the retired pattern). The `/notifications/unread_count` endpoint
  remains available on the server.
